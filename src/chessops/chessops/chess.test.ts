...
test("play move", () => {
  const pos = Chess.fromSetup(
    parseFen("8/8/8/5k2/3p4/8/4P3/4K3 w - -").unwrap()
  ).unwrap();
...
test("castling moves", () => {
  let pos = Chess.fromSetup(
    parseFen("2r5/8/8/8/8/8/6PP/k2KR3 w K -").unwrap()
  ).unwrap();
  ...
  expect(pos.isLegal(move)).toBe(true);
  pos.play(move);
  expect(makeFen(pos.toSetup())).toBe(
...
test.each(insufficientMaterial)(
...
test("impossible checker alignment", () => {
  ...
  const r1 = Chess.fromSetup(
    parseFen("3R4/8/q4k2/2B5/1NK5/3b4/8/8 w - - 0 1").unwrap()
  );
  expect(
    r1.unwrap(
      (_) => undefined,
  ...
  Chess.fromSetup(
    parseFen("8/8/5k2/p1q5/PP1rp1P1/3P1N2/2RK1r2/5nN1 w - - 0 3").unwrap()
  ).unwrap();

  // En passant square aligned with checker and king.
  const r2 = Chess.fromSetup(
    parseFen("8/8/8/1k6/3Pp3/8/8/4KQ2 b - d3 0 1").unwrap()
  );
  expect(
    r2.unwrap(
      (_) => undefined,
...
test("king captures unmoved rook", () => {
  const pos = Chess.fromSetup(
    parseFen("8/8/8/B2p3Q/2qPp1P1/b7/2P2PkP/4K2R b K - 0 1").unwrap()
  ).unwrap();
  ...
  pos.play(move);
  expect(makeFen(pos.toSetup())).toBe(
    "8/8/8/B2p3Q/2qPp1P1/b7/2P2P1P/4K2k w - - 0 2"
  );
});
