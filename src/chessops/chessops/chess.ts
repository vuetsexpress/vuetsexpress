...
export class Castles {
  ...
  static default(): Castles {
    ...
    castles.path = {
      ...
      black: {
        a: new SquareSet(0, 0x0e000000),
        h: new SquareSet(0, 0x60000000),
      },
    };
    return castles;
  }
...
export abstract class Position {
  ...
  ctx(): Context {
    ...
    const snipers = rookAttacks(king, SquareSet.empty())
      ...
      .union(
        bishopAttacks(king, SquareSet.empty()).intersect(
          this.board.bishopsAndQueens()
        )
      )
  ...
  equalsIgnoreMoves(other: Position): boolean {
    return (
      ...
      ((other.pockets && this.pockets?.equals(other.pockets)) ||
      ...
      ((other.remainingChecks &&
        this.remainingChecks?.equals(other.remainingChecks)) ||
  ...
  isLegal(move: Move, ctx?: Context): boolean {
    if (isDrop(move)) {
      ...
      if (
        !!move.promotion !==
        (this.board.pawn.has(move.from) && SquareSet.backranks().has(move.to))
      )
        return false;
  ...
  play(move: Move): void {
    ...
    if (this.remainingChecks) {
      if (this.isCheck())
        this.remainingChecks[turn] = Math.max(
          this.remainingChecks[turn] - 1,
          0
        );
    }
  }
...
export class Chess extends Position {
  ...
  protected validate(opts?: FromSetupOpts): Result<undefined, PositionError> {
    ...
    return opts?.ignoreImpossibleCheck
      ? Result.ok(undefined)
  ...
  private validEpSquare(square: Square | undefined): Square | undefined {
    ...
    if (
      ...
      !this.board[opposite(this.turn)].has(pawn)
    )
      return;
    return square;
  }
  ...
  dests(square: Square, ctx?: Context): SquareSet {
    ...
    let pseudo, legal;
    if (piece.role === "pawn") {
      pseudo = pawnAttacks(this.turn, square).intersect(
        this.board[opposite(this.turn)]
      );
      const delta = this.turn === "white" ? 8 : -8;
      const step = square + delta;
      if (0 <= step && step < 64 && !this.board.occupied.has(step)) {
  ...
  hasInsufficientMaterial(color: Color): boolean {
    ...
    if (this.board[color].intersects(this.board.bishop)) {
      ...
      return (
        sameColor && this.board.pawn.isEmpty() && this.board.knight.isEmpty()
      );
    }
    return true;
  }
