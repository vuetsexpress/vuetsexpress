...
export class Crazyhouse extends Chess {
  ...
  protected validate(opts?: FromSetupOpts): Result<undefined, PositionError> {
    return super.validate(opts).chain((_) => {
      ...
      if (
        (this.pockets ? this.pockets.count() : 0) + this.board.occupied.size() >
        64
      ) {
...
export class Atomic extends Chess {
  ...
  protected kingAttackers(
    ...
    if (
      attackerKings.isEmpty() ||
      kingAttacks(square).intersects(attackerKings)
    ) {
      ...
      if (
        ...
        if (
          !this.board.bishop
            ...
            .intersects(SquareSet.darkSquares())
        ) {
...
export class Antichess extends Chess {
  ...
  hasInsufficientMaterial(color: Color): boolean {
    if (this.board.occupied.equals(this.board.bishop)) {
      ...
      const theyAllOnDark = this.board[opposite(color)].isDisjoint(
        SquareSet.lightSquares()
      );
      const theyAllOnLight = this.board[opposite(color)].isDisjoint(
        SquareSet.darkSquares()
      );
      return (
...
export class ThreeCheck extends Chess {
  ...
  static fromSetup(
    ...
    return super.fromSetup(setup, opts).map((pos) => {
      pos.remainingChecks = setup.remainingChecks
        ? setup.remainingChecks.clone()
        : RemainingChecks.default();
      return pos;
    });
  }
...
export class RacingKings extends Chess {
  ...
  isVariantEnd(): boolean {
    ...
    if (defined(blackKing)) {
      ...
      for (const target of kingAttacks(blackKing)
        .intersect(goal)
  ...
  protected validate(opts?: FromSetupOpts): Result<undefined, PositionError> {
    ...
    for (const color of COLORS) {
      if (
        this.board
          .pieces(color, "pawn")
          .intersects(SquareSet.backrank(opposite(color)))
      ) {
        return Result.err(new PositionError(IllegalSetup.PawnsOnBackrank));
      }
    }
