...
test("make crazyhouse variation", () => {
  const setup = parseFen(
    "r4b1N~/1ppk1P2/p1b5/6p1/8/1PBPPq2/P1PR1P2/1K4N1/PNBRPPPrqnn b - - 71 36"
  ).unwrap();
...
test("make stockfish line with many knight moves", () => {
  const setup = parseFen(
    "2rq1rk1/pb1nbp1p/1pn3p1/3pP3/2pP4/1N3NPQ/PP3PBP/R1B1R1K1 w - - 0 16"
  ).unwrap();
...
test("parse pawn capture", () => {
  ...
  pos = Chess.fromSetup(
    parseFen("r4br1/pp1Npkp1/2P4p/5P2/6P1/5KnP/PP6/R1B5 b - -").unwrap()
  ).unwrap();
