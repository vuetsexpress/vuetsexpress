...
export function parseCastlingFen(
  board: Board,
  castlingPart: string
  ...
  for (const c of castlingPart) {
    ...
    if (lower === "q") candidates = backrank;
      ...
      candidates = SquareSet.fromSquare(
...
export function parseRemainingChecks(
  part: string
...
export function parseFen(fen: string): Result<Setup, FenError> {
  ...
  return board.chain((board) => {
    ...
    const fullmoves = defined(fullmovesPart)
    ...
    let remainingChecks: Result<RemainingChecks | undefined, FenError> =
      Result.ok(undefined);
...
export function makeCastlingFen(
  board: Board,
