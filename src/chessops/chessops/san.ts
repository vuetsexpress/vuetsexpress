    ...
    if (
      ...
      if (role !== "pawn") {
        ...
        if (role === "king")
          ...
          others = queenAttacks(move.to, pos.board.occupied).intersect(
            pos.board.queen
          );
        else if (role === "rook")
          others = rookAttacks(move.to, pos.board.occupied).intersect(
            pos.board.rook
          );
        else if (role === "bishop")
          others = bishopAttacks(move.to, pos.board.occupied).intersect(
            pos.board.bishop
          );
        ...
        if (others.nonEmpty()) {
          ...
          if (others.nonEmpty()) {
            ...
            let column = others.intersects(
              SquareSet.fromRank(squareRank(move.from))
            );
...
export function parseSan(pos: Position, san: string): Move | undefined {
  ...
  const pawnAdvance =
    role === "pawn" ? SquareSet.fromFile(squareFile(to)) : SquareSet.empty();
  candidates = candidates.intersect(
    pawnAdvance.union(
      attacks({ color: opposite(pos.turn), role }, to, pos.board.occupied)
    )
  );
