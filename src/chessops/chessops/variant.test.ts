...
const variantPerfts: [Rules, string, string, number, number, number][] = [
  ...
  [
    "3check",
    "castling",
    "r3k2r/8/8/8/8/8/8/R3K2R w KQkq - 1+1",
    26,
    562,
    13410,
  ],
...
test.each(variantPerfts)(
  "variant perft: %s (%s): %s",
  (rules, name, fen, d1, d2, d3) => {
...
test.each(insufficientMaterial)(
  "%s insufficient material: %s",
  (rules, fen, white, black) => {
    const pos = setupPosition(rules, parseFen(fen).unwrap()).unwrap();
...
test("king of the hill not over", () => {
  const pos = setupPosition(
    "kingofthehill",
    parseFen(
      "rnbqkbnr/pppppppp/8/1Q6/8/8/PPPPPPPP/RNB1KBNR w KQkq - 0 1"
    ).unwrap()
  ).unwrap();
...
test("racing kings end", () => {
  ...
  const black = setupPosition(
    "racingkings",
    parseFen("1k6/6K1/8/8/8/8/8/8 w - - 0 1").unwrap()
  ).unwrap();
  ...
  const pos = setupPosition(
    "racingkings",
    parseFen("1K6/7k/8/8/8/8/8/8 b - - 0 1").unwrap()
  ).unwrap();
  expect(pos.isEnd()).toBe(false);
  expect(pos.outcome()).toBeUndefined();

  // Black near backrank but cannot move there.
  const white = setupPosition(
    "racingkings",
    parseFen("2KR4/k7/2Q5/4q3/8/8/8/2N5 b - - 0 1").unwrap()
  ).unwrap();
